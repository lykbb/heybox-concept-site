...
视觉参考：[KPR](https://www.kprverse.com/) 的超大粗字与错位排版。各段主标题、产品卡标题及大字装饰优先调用本机已安装的「优设标题黑」，其他设备会使用系统粗体回退；未将字体文件打包进网站。第三段的图标散开与逐行文字参考用户提供的 [Mobbin](https://mobbin.com/) 页面截图；第二段与第三段之间采用约 800ms 的双向整屏切换。切屏时图标错开散布四周，四行文案在约 1.5 秒内自动依次出现；之后图标独立缓慢漂浮，离开该段即暂停，并兼容系统“减少动态效果”设置。
...
- `assets/hero-gameplay-web.mp4`：当前首页使用的网页播放版本；本地原片 `assets/hero-gameplay.mp4` 保留在项目目录，不上传。`assets/hero-temp.mp4` 是此前来自 [Pixabay / spacetrip](https://pixabay.com/videos/videogame-space-science-fiction-154881/) 的临时视频，按 [Pixabay Content License](https://pixabay.com/service/license-summary/) 用于概念样稿。
- `assets/heybox-official-logo.svg`：从[小黑盒官网](https://xiaoheihe.cn/)页眉提取的原版矢量标志。
